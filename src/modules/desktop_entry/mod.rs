...
const DESKTOP_FILE: &str = "halvora.desktop";
...
pub fn icon_path() -> Option<PathBuf> {
    let data = dirs::data_dir()?;
    Some(data.join("icons").join("halvora.svg"))
}
...
pub fn ensure() {
    #[cfg(target_os = "linux")]
    {
        ...
        let Some(entry_path) = desktop_file_path() else {
            eprintln!("[halvora] warning: could not determine home for desktop entries");
            return;
        };
        let Some(target_icon) = icon_path() else {
            eprintln!("[halvora] warning: could not determine home for app icons");
            return;
        };
        ...
        if let Err(e) = fs::write(&target_icon, ICON_SVG) {
            eprintln!("[halvora] warning: could not write app icon {target_icon:?}: {e}");
            return;
        }
        ...
        if let Err(e) = fs::write(&entry_path, desktop_content) {
            eprintln!("[halvora] warning: could not write desktop entry {entry_path:?}: {e}");
            return;
        }
        ...
        eprintln!("[halvora] desktop entry {action}: {}", entry_path.display());
    }
}
